...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) external {
        deposit(msg.sender, _pid, _wantAmt);
    }

    // Want tokens moved from user -> NativeFarm (NATIVE allocation) -> Strat (compounding)
    function deposit(address _userAddress, uint256 _pid, uint256 _wantAmt) public {
        updatePool(_pid);
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function _harvestNativeTokens(address _userAddress, PoolInfo memory _pool, UserInfo memory _user) internal {
        uint256 pending =
            _user.shares.mul(_pool.accNATIVEPerShare).div(1e12).sub(
                _user.rewardDebt
            );
        if (pending > 0) {
    ...
    function setMintParameters(uint256 _NATIVEPerBlock) external onlyOwner {
        require(_NATIVEPerBlock < NATIVEPerBlock, "New value must be lower");
