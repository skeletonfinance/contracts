...
abstract contract Strategy is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public earlyWithdrawDuration = 0 days;
    uint256 public earlyWithdrawFeeFactor = 100;
    uint256 public constant earlyWithdrawFeeFactorMax = 10000;
    uint256 public constant earlyWithdrawFeeFactorUL = 500;
        ...
        returns (uint256)
    {
        ...
        _depositedAt[_userAddress] = block.timestamp;
        ...
        returns (uint256)
    {
        ...
        uint256 earlyWithdrawFee = earlyWithdrawalFee(_wantAmt, _depositedAt[_userAddress]);
        if (withdrawFeeFactor < withdrawFeeFactorMax)
            _wantAmt = _wantAmt.mul(withdrawFeeFactor).div(withdrawFeeFactorMax);

        if (earlyWithdrawFee > 0)
            _wantAmt -= earlyWithdrawFee;
    ...
    function earlyWithdrawalFee(uint amount, uint depositedAt) internal view returns (uint256) {
        if (earlyWithdrawFeeFactor > 0 && depositedAt.add(earlyWithdrawDuration) > block.timestamp)
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        uint256 totalBuyBackRate = vaultBuyBackRate + buyBackRate;
        if (totalBuyBackRate <= 0) {
            return _earnedAmt;
        }

        uint256 buyBackAmt = _earnedAmt.mul(totalBuyBackRate).div(buyBackRateMax);
    ...
    function _sendBuyBack() internal virtual {
        uint256 earnedAmt = IERC20(NATIVEAddress).balanceOf(address(this));
        uint256 burnBuyBackAmt = earnedAmt.mul(buyBackRate).div(vaultBuyBackRate + buyBackRate);
        uint256 vaultBuyBackAmt = earnedAmt - burnBuyBackAmt;

        if (burnBuyBackAmt > 0) {
            IERC20(NATIVEAddress).safeTransfer(buyBackAddress, burnBuyBackAmt);
        }

        if(vaultBuyBackAmt > 0) {
            IERC20(NATIVEAddress).safeTransfer(vaultBuyBackAddress, vaultBuyBackAmt);
        }
    }

    function setVaultBuyBack(uint256 _vaultBuyBackRate, address _vaultBuyBackAddress) public virtual onlyAllowGov {
        require(_vaultBuyBackRate + buyBackRate <= buyBackRateUL, "too high");
        vaultBuyBackRate = _vaultBuyBackRate;
        vaultBuyBackAddress = _vaultBuyBackAddress;
    }
    ...
    function setEarlyWithdrawFeeFactor(uint256 _earlyWithdrawFeeFactor) public onlyAllowGov {
    ...
    function setBuybackRouterAddress(address _buybackRouterAddress) public virtual onlyAllowGov {
